...
// ==========================================
// --- ROTAS DE ARQUIVOS (NOVA) ---
// ==========================================

// Rota para visualizar a imagem/vídeo do material
app.get('/api/materiais/arquivo/:id', (req, res) => {
    ...
    db.query(sql, [id], (err, results) => {
        ...
        // Define o tipo do conteúdo (ex: image/png, video/mp4) para o navegador entender
...
app.post('/api/cadastrar', upload.single('arquivo'), (req, res) => {
    ...
    db.query(queryIDs, (err, results) => {
        ...
        const sqlInsert = `INSERT INTO materiais (id, nome_item, destino, projeto, observacoes, arquivo_dados, arquivo_tipo, arquivo_nome) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;
        
        db.query(sqlInsert, [novoId, item, destino, projeto, observacoes, arquivoDados, arquivoTipo, arquivoNome], (err, result) => {
...
app.put('/api/atualizar', upload.single('arquivo'), (req, res) => {
    ...
    if (req.file) {
        sql = `UPDATE materiais SET nome_item = ?, destino = ?, projeto = ?, observacoes = ?, arquivo_dados = ?, arquivo_tipo = ?, arquivo_nome = ? WHERE id = ?`;
        params = [item, destino, projeto, observacoes, req.file.buffer, req.file.mimetype, req.file.originalname, id];
    } else {
        sql = `UPDATE materiais SET nome_item = ?, destino = ?, projeto = ?, observacoes = ? WHERE id = ?`;
        params = [item, destino, projeto, observacoes, id];
    }
...
app.get('/api/materiais', (req, res) => {
    // Retorna metadados do arquivo (nome/tipo) mas NÃO o blob pesado
    const sql = 'SELECT id, nome_item, destino, projeto, observacoes, arquivo_nome, arquivo_tipo FROM materiais ORDER BY id ASC';
