  ...
  try {
    ...
    if (filtroSetor) {
      projetos = projetos.filter((projeto) => {
        ...
        if (termo === "manutencao" && setorProj.includes("manutencao"))
          return true;
...
function renderizarProjetos(projetos, materiais) {
  ...
  projetos.forEach((projeto) => {
    ...
    const precoFormatado = projeto.preco
      ? parseFloat(projeto.preco).toLocaleString("pt-BR", {
          style: "currency",
          currency: "BRL",
...
function configurarAcaoToggle() {
  ...
  container.addEventListener("click", (event) => {
    ...
    const projetoAlvo = listaGlobalProjetos.find(
      (p) => String(p.id) === String(idProjeto)
    );
    ...
    const linhasHTML = materiaisParaExibir
      .map(
        (mat) => `
            <tr>
                <td>${String(mat.id).padStart(3, "0")}</td>
